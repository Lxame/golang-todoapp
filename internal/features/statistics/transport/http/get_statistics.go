...
// @Param        form    query    string             false         "Начало промежутка рассмотрения статистики (включительно), формат: YYYY-MM-DD"
// @Param        to      query    string             false         "Конец промежутка рассмотрения статисикик (не включительно), формат: YYYY-MM-DD"
...
func toDTOFromDomain(statistics core_domain.Statistics) GetStatisticsResponse {
	var avgTime *string
...
func getUserIDFromToQueryParams(
	...
	const (
		...
		fromQueryParamKey   = "limit"
		toQueryParamKey     = "offset"
	)
